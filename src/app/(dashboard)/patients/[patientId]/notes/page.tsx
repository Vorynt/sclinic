import { ForbiddenBlock } from "@/components/status/ForbiddenBlock"
import { Permission } from "@/config/permissions"
import { PatientClinicalNotesSection } from "@/modules/medical-records/components/PatientClinicalNotesSection"
import { PermissionProvider } from "@/providers/PermissionProvider"

type PatientNotesPageProps = {
  params: Promise<{ patientId: string }>
}

export default async function PatientNotesPage({
  params,
}: PatientNotesPageProps) {
  const { patientId } = await params

  return (
    <PermissionProvider
      permission={Permission.RECORDS_READ}
      fallback={<ForbiddenBlock />}
    >
      <PatientClinicalNotesSection patientId={patientId} />
    </PermissionProvider>
  )
}
