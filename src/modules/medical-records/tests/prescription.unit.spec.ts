...
describe("canEditPrescription", () => {
  it("allows editing only while attendance is checked_in", () => {
    expect(canEditPrescription("checked_in")).toBe(true)
    expect(canEditPrescription("completed")).toBe(false)
    expect(canEditPrescription("scheduled")).toBe(false)
  })
})
...
describe("sanitizePrescriptionHtml", () => {
  it("strips script tags and event handlers", () => {
    ...
    expect(clean.includes("javascript:")).toBe(false)
...
describe("renderPrescriptionHtml", () => {
  it("replaces placeholders with snapshot values and body", () => {
    ...
    expect(html).toMatch(/Clínica Exemplo/)
    expect(html).toMatch(/Maria Silva/)
    expect(html).toMatch(/Dipirona 500mg/)
    expect(html).toMatch(/CRM 12345\/SP/)
...
describe("prescription schemas", () => {
  it("accepts create payload", () => {
    ...
    expect(parsed.plainText).toBe("Receita")
  })

  it("rejects empty body", () => {
    expect(() => createPrescriptionSchema.parse({
              appointmentId: VALID_UUID,
              body: "   ",
              plainText: "   ",
            })).toThrow()
  ...
  it("accepts issue and create layout payload", () => {
    expect(issuePrescriptionSchema.parse({ id: VALID_UUID }).id).toBe(VALID_UUID)
    expect(createPrescriptionSchema.parse({
        appointmentId: VALID_UUID,
        body: "<p>Receita</p>",
        plainText: "Receita",
        layoutId: VALID_UUID,
      }).plainText).toMatch(/Receita/)
  })
})
...
describe("toPrescription", () => {
  it("maps repository row to domain type", () => {
    ...
    expect(prescription.kind).toBe("prescription")
    expect(prescription.plainText).toBe("x")
    expect(prescription.layoutId).toBe(null)
  })
})
