...
import { deleteScheduleBlockSchema } from "@/modules/appointments/schemas/schedule-block.schema"
...
export async function deleteScheduleBlockAction(
  ...
  return toActionResult(async () => {
    const parsed = parseOrThrow(deleteScheduleBlockSchema, data)
    return scheduleBlockService.remove(parsed.id, await getAuthRequestContext())
  })
}
