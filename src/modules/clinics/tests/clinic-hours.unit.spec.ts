...
describe("clinicWeeklyHoursSchema", () => {
  it("accepts a full week with two intervals", () => {
    ...
    expect(parsed.days[1]?.intervals.length).toBe(2)
  })
...
describe("zonedWallTimeToUtc", () => {
  it("maps America/Sao_Paulo wall clock to the correct UTC instant", () => {
    ...
    expect(instant.toISOString()).toBe("2026-07-23T12:00:00.000Z")
  })
})

describe("formatDayHoursSummary", () => {
  it("returns Fechado when closed or empty", () => {
    expect(formatDayHoursSummary({ isClosed: true, intervals: [] })).toBe("Fechado")
    expect(formatDayHoursSummary({ isClosed: false, intervals: [] })).toBe("Fechado")
  })

  it("joins one or two intervals", () => {
    expect(formatDayHoursSummary({
        isClosed: false,
