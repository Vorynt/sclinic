...
| Tema | Decisão |
|------|--------|
| Bloqueios | Tabela `schedule_blocks`: `clinicId`, `professionalId` (**nullable** = clínica inteira), `startsAt`/`endsAt`, `reason` opcional, soft-delete. Contam como busy na availability (junto com appointments ≠ `canceled`). Self-schedule (clinician/nurse): só cria/remove o próprio; clinic-wide só operação/gestão |
