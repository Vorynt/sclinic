...
### E6 — Recebíveis
Charge 1:1, markPaid/cancel, listagem `/billing`, collect vs view. Catálogo de serviços em `/services`, desconto %, cortesia/retorno (ADR-009). **Done (ADR-011):** `dueAt` por cobrança + filtro de vencidas em `/billing`. Painel `/billing` com KPIs, gráficos, filtros (período = data da consulta) e exportação CSV/impressão. Gateway → E10. Comissão → extensão no [Roadmap](Roadmap).
